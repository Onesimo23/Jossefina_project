<div class="bg-gray-50 dark:bg-zinc-900 min-h-screen">
    ...
    <section class="relative bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 dark:from-zinc-800 dark:via-zinc-700 dark:to-zinc-600 pt-20 pb-32 overflow-hidden">
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center">
                <h1 class="text-5xl md:text-7xl font-extrabold text-white mb-6 drop-shadow-2xl animate-fade-in-down">
                    🌟 Extensão Universitária
                </h1>
                <p class="text-xl md:text-3xl text-indigo-100 dark:text-indigo-200 max-w-4xl mx-auto mb-8 leading-relaxed animate-fade-in-up">
                    Conectando <span class="font-bold text-yellow-300">conhecimento acadêmico</span> com <span class="font-bold text-yellow-300">transformação social</span>
                </p>
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center mb-16 animate-fade-in-up animation-delay-400">
                    @auth
                        ...
                        @endif
                    @else
                        <a href="{{ route('register') }}" class="inline-flex items-center justify-center px-8 py-4 bg-white text-indigo-600 font-bold rounded-full shadow-2xl hover:shadow-3xl hover:scale-105 transition-all duration-300">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto">
                    <div class="bg-white/10 backdrop-blur-lg rounded-3xl p-6 transform hover:scale-105 transition-all duration-300 border border-white/20">
                        <div class="text-5xl font-extrabold text-white mb-2" x-data="{ count: 0 }" x-init="setInterval(() => { if(count < {{ $activities->count() }}) count++ }, 30)">
                            <span x-text="count"></span>
                        </div>
                        <div class="text-white/90 font-medium text-sm">Atividades Disponíveis</div>
                    </div>
                    @auth
                        @if(auth()->user()->role === 'community')
                            <div class="bg-white/10 backdrop-blur-lg rounded-3xl p-6 transform hover:scale-105 transition-all duration-300 border border-white/20">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse ($activities as $activity)
                    <div class="group relative bg-white dark:bg-zinc-700 rounded-3xl shadow-xl hover:shadow-2xl overflow-hidden border-2 border-gray-100 dark:border-zinc-600 cursor-pointer transform hover:-translate-y-3 transition-all duration-500"
                         wire:click="openActivityModal({{ $activity->id }})">

                        {{-- Card Header with Gradient --}}
                        <div class="h-56 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 relative overflow-hidden">
                            {{-- Overlay Pattern --}}
                            <div class="absolute inset-0 opacity-20">
            ...
            @if ($selectedActivity)
                <div class="bg-white dark:bg-zinc-800 rounded-3xl shadow-2xl overflow-hidden border-2 border-gray-100 dark:border-zinc-700">
                    ...
                    <div class="h-64 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 relative overflow-hidden">
                        ...
                        {{-- Close Button --}}
                        <button wire:click="closeModal" class="absolute top-4 right-4 w-12 h-12 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-full flex items-center justify-center text-white transition-all duration-300 hover:scale-110">
                        ...
                        <div class="position-absolute top-3 start-3">
                            <span class="badge bg-light bg-opacity-75 text-primary rounded-pill px-3 py-2 shadow">
                                {{ $selectedActivity->category->name ?? 'Sem categoria' }}
                            </span>
                        </div>

                        {{-- Activity Title --}}
                        <div class="position-absolute bottom-0 start-0 end-0 p-4 bg-gradient">
                            <h3 class="display-6 text-white mb-2">{{ $selectedActivity->title }}</h3>
                            <div class="d-flex gap-4 text-white-50">
                                <div class="d-flex align-items-center">
                                    <i class="bi bi-calendar-event me-2"></i>
                                    <span>{{ optional($selectedActivity->starts_at)->format('d/m/Y H:i') ?? 'Data a definir' }}</span>
                                </div>
                                <div class="d-flex align-items-center">
                                    <i class="bi bi-geo-alt me-2"></i>
                                    <span>{{ $selectedActivity->location ?? 'Local a definir' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-4">
                        <div class="mb-4">
                            {!! nl2br(e($selectedActivity->description)) !!}
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="card bg-light h-100">
                                    <div class="card-body">
                                        ...
                                        <p class="card-text">{{ $selectedActivity->organizer->name ?? 'Não informado' }}</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="card bg-light h-100">
                                    <div class="card-body">
                                        ...
                                        <p class="card-text">{{ $selectedActivity->available_slots ?? 'Ilimitado' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex flex-column flex-sm-row gap-3">
                            @auth
                                @if($selectedActivity->is_enrolled)
