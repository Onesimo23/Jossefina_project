<div>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Gestão de Atividades</h2>
    </x-slot>
    <div class="py-12">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr wire:key="activity-{{ $activity->id }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex space-x-2">
                                    ...
                                    {{-- @endcan --}}
                                    {{-- CHAMADA PARA ABRIR O CHAT --}}
                                    <a wire:click="openChat({{ $activity->id }})" class="text-blue-600 hover:text-blue-900 cursor-pointer">Chat</a>
                                    {{-- @can('delete', $activity) --}}
    ...
    <div
        ...
        <div x-show="show" class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm transition-opacity duration-300"
        ...
        <div
            ...
            <div class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:w-full sm:max-w-4xl"
                @click.outside="$wire.closeChat()"
                @keydown.escape.window="$wire.closeChat()">

                @if($chatActivityId)
                    @php
                        // Procura a atividade usando o ID definido pelo método openChat
                        $chatActivity = \App\Models\Activity::find($chatActivityId);
                    @endphp

                    <div class="p-0">
                        {{-- O key() força a re-renderização do componente filho quando o ID muda --}}
                        @livewire('activity.activity-chat', ['activity' => $chatActivity], key('chat-'.$chatActivityId))
                    </div>
                @else
                    <div class="p-6 text-center text-gray-500">
                        Nenhuma atividade selecionada para o chat.
                    </div>
                @endif
